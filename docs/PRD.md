...
| FR-2.3 | 对象树**按图元类型分组**(元件/文本/焊盘/走线…),组头显示类型中文名与计数;元件组内只列位号(可重复位号各自独立),其余类型列图元 id;**元件树只列真元件**(v0.3.1 #component-tree:按客户端器件 `symbolType:"component"` 标志过滤——文件存储形态为所引用 SYMBOL 文档 META.docType,2=元件/17=复合块符号算元件,18 电源标志/19 网络端口/20 图框/22 短接/31 差分对不算;属性中直接携带 `symbolType` 时以其为准;Designator 为空的元件如实列出,不以位号有无为判据),按位号自然排序;多页原理图的元件列表跨整张原理图合并,兄弟页只做属性扫描不建渲染场景(#page-switch-jank),切页时行集不变则跳过 DOM 重建 | P0 |
