...
| FR-2.3 | 对象树**按图元类型分组**(元件/文本/焊盘/走线…),组头显示类型中文名与计数;元件组内只列位号(可重复位号各自独立),其余类型列图元 id;**元件树只列真元件**(v0.3.1 #component-tree:仅列携带 Designator 的 `COMPONENT`——实例 ATTR 或器件 META 默认值任一即可;netport/电源符号/图框在两类来源里都没有 Designator,x86-pc CPU 页 100 个 COMPONENT 实测过滤掉 98 个 netport + 1 个图框),按位号自然排序;多页原理图的元件列表跨整张原理图合并,兄弟页只做属性扫描不建渲染场景(#page-switch-jank),切页时行集不变则跳过 DOM 重建 | P0 |
