...
| FR-3.14 | **图层激活**:点击图层面板行将活跃层组提至栈顶(置顶显示,内层/对面铜可见);常驻工具层(multi/板框/轴/飞线/钻孔)压回原序;对面标注浮层沉回不遮活跃层;每面独立的网络名(nn)/编号(pn)/铺铜(pour)合成子层组随活跃面整体提层,活跃层切换高亮;激活层同时是画布点选的**优先拾取层**(#pick-active-layer,行为细则见 FR-4.3),重置图层/切换文档时拾取优先一并清除 | P0 |
...
| FR-4.3 | 左键点选图元 → 选中(描边+控制框),命中容差随缩放级别自适应;空白处点击取消选择;**活跃层优先拾取**(#pick-active-layer,v0.3.4):PCB 族文档(PCB/面板/封装)设了激活层时,画布点击命中多个重叠图元优先选**激活层上 bbox 最小**的图元,激活层无命中或未设激活层时回退既有"整体 bbox 最小面积"规则,原理图文档行为不变;命中与隐藏层判定以渲染时落定的层组 key(RenderObject.layerKey,含 pour:/pn:/nn: 合成组与元件归层)为准;**选中高亮贴合路径**(#select-box-path,v0.3.4):线类图元(原理图导线 LINE/折线 POLY、PCB 走线 LINE/圆弧 ARC/折线 POLY)的选中高亮沿**实际绘制路径**描亮(圆弧按 arcPts/arc3Pts 采样为折线,与绘制的 SVG 弧逐点吻合),不再取整体 bbox 最大矩形——斜走线/斜导线不再出现大角度失配的粗斜框,与 EDA 客户端沿线高亮习惯一致;其余图元(元件/焊盘/文本/铺铜等)保持 bbox 控制框;高亮线恒定像素虚线(线宽 2px、4 开 3 关,随缩放恒定不变粗,#select-box-path) | P0 |
